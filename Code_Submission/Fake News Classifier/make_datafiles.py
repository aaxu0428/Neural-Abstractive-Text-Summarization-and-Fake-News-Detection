...
def write_to_bin(out_file, makevocab=False):
  ...
  # url_file is like all_train.txt for example
  # url_list = read_text_file(url_file) #this gives you list of urls
  # url_hashes = get_url_hashes(url_list) #gives you the hash of the url
  story_fnames = [os.path.join(dirpath, f) for dirpath, dirnames, files in os.walk(dm_stories_dir) for f in files if f.endswith('.txt')]
  # story_fnames = [s+".story" for s in url_hashes] #file names of each story (IMPORTANT)
  ...
  with open(out_file, 'wb') as writer:
    for idx,s in enumerate(story_fnames):
      ...
      if os.path.isfile(os.path.join(dm_tokenized_stories_dir, s)):
...
if __name__ == '__main__':
  # if len(sys.argv) != 3:
    # print "USAGE: python make_datafiles.py <dailymail_stories_dir>"
    # sys.exit()
  # cnn_stories_dir = sys.argv[1]
  dm_stories_dir = sys.argv[1]
